...
from experiments import prostate_scribble_full_diagonal as exp_config
...
#        with tf.Graph().as_default():
        with tf.Session(config = config) as sess:
            ...
            logging.info(str(crf_variables))
            
            spatial_weight = []
            for v in crf_variables:

                if v.name[10:26] == 'spatial_weight_5':
                    spatial_weight.append(v)         
                    print("FOUNDDDDDDD")
                if v.name[10:27] == 'spatial_weight_10':
                    spatial_weight.append(v)
                    print("FOUNDDDDDDD")


            spatial_ker_weights = None
            for v in crf_variables:
                if v.name[10:21] == 'spatial_ker':
                    spatial_ker_weights = v
...
#            reg_loss = tf.pow(spatial_weight[0], 2) + tf.pow(spatial_weight[1], 2) 
#            loss = loss + reg_loss
...
#            saver.restore(sess,'/scratch_net/biwirender02/cany/scribble/logdir//recursion_0_model_best_dice.ckpt-14599')
            step = init_step
...
#                        print("CRF variables : " + str(sess.run(crf_variables)))
                        logging.info("Spatial weights : " + str(sess.run(spatial_weight)))
                        logging.info("Spatial_ker_weights : " + str(sess.run(spatial_ker_weights)))
