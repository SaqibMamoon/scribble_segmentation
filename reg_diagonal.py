...
from experiments import diagonal_min_10_exp as exp_config
...
#        with tf.Graph().as_default():
        with tf.Session(config = config) as sess:
            ...
            spatial_weight = []
            for v in crf_variables:
#                if v.name[10:26] == 'spatial_weight_0':
#                    spatial_weight.append(v)
#                    print("FOUNDDDDDDD")
                if v.name[10:26] == 'spatial_weight_6':
                    spatial_weight.append(v)         
                    print("FOUNDDDDDDD")
                if v.name[10:27] == 'spatial_weight_12':
                    spatial_weight.append(v)
                    print("FOUNDDDDDDD")
                if v.name[10:27] == 'spatial_weight_18':
                    spatial_weight.append(v)
                    print("FOUNDDDDDDD")
#                if v.name[10:27] == 'spatial_weight_24':
#                    spatial_weight.append(v)
#                    print("FOUNDDDDDDD")
#            crf_weights = []
#            crf_weights_name = []
...
#                    crf_weights_name.append(v.name)
            ...
            reg_loss = tf.nn.l2_loss(spatial_weight)
            crf_train_op = tf.train.AdamOptimizer(learning_rate=crf_learning_rate_placeholder).minimize(loss + 0.00001*reg_loss,var_list=crf_variables,
                                                                                         colocate_gradients_with_ops=True,
                                                                                         global_step=global_step)
...
#            saver.restore(sess,'/scratch_net/biwirender02/cany/scribble/logdir/twisted_full_network/recursion_2_model_best_dice.ckpt-43499')
            step = init_step
...
#                        print("CRF variables : " + str(sess.run(crf_variables)))
                        print("Spatial weights : " + str(sess.run(spatial_weight)))
                    if (step % 10 == 0) :
