...
def apply_crf(seg_pred_probs,imgs, parameters):
...
def score_data(input_folder, output_folder, model_path, exp_config, do_postprocessing=False, recursion=None):
    ...
    images = np.array(base_data['images_val'])
    labels= np.array(base_data['masks_val'])
...
#                                                                         'recursion_{}_model.ckpt'.format(recursion))
        ...
        checkpoint_path = os.path.join(model_path , 'recursion_0_model_best_dice.ckpt-5399')
                ...
                for n in range(len(values3)):
                    for o in range(len(values4)):
                        ...
                        #            y=labels[int(sli+cur_slice),:,:]
                                    ...
                                    mask_op = tf.nn.softmax(logits_op)
                    #                mask_op = tf.arg_max(softmax, dimension=-1)  # was 3
                                    ...
                                    mask = np.squeeze(apply_crf(mask,x,parameters))
                        ...
                        #            dice = dc(gt_c_i, pred_c_i)
                            ...
                            cla_di = np.mean(dices,0)
                            logging.info("Avg dice : " + str(cla_di[0]) + " and " +str(cla_di[1]))
                            text_file = open('/home/cany/pro_crf_grid_search.txt', "a")
                            text_file.write("\nParameters " + str(parameters) +" : " + str(avg_dice))
                            text_file.close()
    return 0
...
if __name__ == '__main__':
    ...
    model_path = '/scratch_net/biwirender02/cany/scribble/logdir/prostate_scribble_full_deep_base'
