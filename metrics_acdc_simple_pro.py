...
def compute_metrics_on_directories(dir_gt, dir_pred):
    ...
    return [dice1, dice2,  vold1, vold2]
...
def compute_metrics_on_directories_raw(dir_gt, dir_pred):
    ...
    for p_gt, p_pred in zip(lst_gt, lst_pred):
        ...
        res_mat[ind, :6] = metrics(gt, pred, zooms)

        for ii, struc in enumerate([1,2]):

            gt_binary = (gt == struc) * 1
            pred_binary = (pred == struc) * 1

            res_mat[ind, 6+ii] = hd(gt_binary, pred_binary, voxelspacing=zooms, connectivity=1)
            res_mat[ind, 8+ii] = assd(pred_binary, gt_binary, voxelspacing=zooms, connectivity=1)
...
#        print_less_stats(df, eval_dir)
        [dice1, dice2,  vold1, vold2] = compute_metrics_on_directories(path_gt, path_pred)

        logging.info('------------Average Dice Figures----------')
        logging.info('Dice 1: %f' % dice1)
        logging.info('Dice 2: %f' % dice2)
        logging.info('Mean dice: %f' % np.mean([dice1, dice2]))
        logging.info('------------------------------------------')
