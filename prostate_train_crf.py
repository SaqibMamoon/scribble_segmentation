...
from experiments import prostate_full_rw90_crf_exp as exp_config
...
#                                                                     exp_config.random_walk)
                    #get ground truths
                    labels_train = np.array(recursion_data['postprocessed'])
...
#                        last_train = train_loss
                    ...
                    if (step + 1) % exp_config.val_eval_frequency == 0:
                        ...
                        [val_loss, val_dice, hard_pred, labels] = do_eval(sess,
                                                       eval_val_loss,
                                                       images_placeholder,
                                                       labels_placeholder,
                                                       training_time_placeholder,
...
def do_eval(sess,
    ...
    # but at the risk of not catching exceptions
        ...
        closs, cdice,hard_pred, labels,l,r = sess.run(eval_loss, feed_dict=feed_dict)
        

        for k in range(cdice.shape[-1]):
            for m in range(cdice.shape[0]):
                if ((l[m,k] == 0) & (r[m,k] == 0)):
                    cdice[m,k]=  1
        
        all_pred.append(hard_pred)
        all_labels.append(labels)
        loss_ii += closs
        dice_ii += np.mean(cdice)
        num_batches += 1
    ...
    return avg_loss, avg_dice, np.asarray(all_pred), np.asarray(all_labels)
