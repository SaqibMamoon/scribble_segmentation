...
import utils
import image_utils
import model_two_outputs as model
from background_generator import BackgroundGenerator
import config.system as sys_config
import acdc_data_two_outputs as acdc_data
...
from experiments import heart_two_outputs_exp as exp_config
            ...
            for epoch in range(start_epoch, exp_config.max_epochs):
                if (epoch % exp_config.epochs_per_recursion == 0 and epoch != 0) \
                    ...
                    recursion_data = predict_next_gt(data=recursion_data,
                                                     ...
                                                     logits=logits,
                                                     network_output_op=network_output,
                                                     sess=sess)
...
#                         .format(ind[0], ind[-1]))
            ...
            net_out = sess.run(network_output_op,feed_dict=feed_dict)
            
            pre_pred = sess.run(tf.nn.softmax(net_out),feed_dict=feed_dict)
            
            #save to dataset
            for indice in range(len(ind)):
                network_output[ind[indice], ...] = np.squeeze(net_out[indice, ...])
                predicted_pre_crf[ind[indice], ...] = np.squeeze(pre_pred[indice, ...])
