...
def score_data(input_folder, output_folder, model_path, exp_config, do_postprocessing=False, recursion=None):
    ...
    net_tensor_shape = [batch_size] + list(exp_config.image_size) + [5]
                    ...
                    for file in glob.glob(os.path.join(folder_path, 'patient???_frame??.nii.gz')):
                        ...
                        if exp_config.data_mode == '2D':
                            ...
                            for zz in range(img.shape[2]):
                                ...
                                n_out = average_outputs(sess,net_out,images_pl,network_input,keep_prob)
                                logits_out = sess.run(softmax_pl,feed_dict={images_pl: network_input,rnn_in_placeholder:n_out})
...
if __name__ == '__main__':
    ...
    model_path = '/scratch_net/biwirender02/cany/scribble/logdir/heart_residual_dropout_rnn_exp'
