...
from experiments import prostate_deep_dropout_rnn_net_mean_exp as exp_config
...
#            saver.restore(sess,'/scratch_net/biwirender02/cany/scribble/logdir/prostate_deep_dropout_rnn_exp/recursion_0_model_best_dice.ckpt-5499')
            saver.restore(sess,"/scratch_net/biwirender02/cany/scribble/logdir/prostate_deep_dropout_rnn_net_mean_exp/recursion_0_model_best_dice.ckpt-4099")

            recursion=0
            init_step = 16000
            start_epoch = exp_config.epochs_per_recursion
            
            step = init_step
            curr_lr = exp_config.learning_rate/20
            crf_curr_lr = 1e-08
...
#            temp = np.zeros((50,exp_config.batch_size,320,320,4))
#            
#            ps = np.zeros((exp_config.batch_size,320,320))
#            for k in range(50):
#                
#
...
#            temp_mean = np.squeeze(np.mean(temp,axis=0))
            ...
            temp_mean =sess.run(logits,feed_dict=feed_dict)
            conf = 0.05
...
#                            mask_out[t,m,n] = 0
            ...
            for indice in range(len(ind)):
                prediction_pre_cor[ind[indice], ...] = np.squeeze(pre_cor_mask[indice, ...])
