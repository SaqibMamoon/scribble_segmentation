...
def inference(images, inference_handle, nlabels, training=True):
    ...
    network_output,s = inference_handle(images, training, nlabels)
    tf.logging.set_verbosity(tf.logging.WARN)
...
def predict_net_out(images, inference_handle, nlabels):
    '''
    Returns the prediction for an image given a network from the model zoo
    :param images: An input image tensor
    :param inference_handle: A model function from the model zoo
    ...
    net_output = model_zoo.seperate_non_net(images, tf.constant(False, dtype=tf.bool), nlabels)
    

    return net_output

def predict_logits(images, inference_handle, nlabels):
    '''
    Returns the prediction for an image given a network from the model zoo
    :param images: An input image tensor
    :param inference_handle: A model function from the model zoo
    ...
    logits = inference_handle(images, tf.constant(False, dtype=tf.bool), nlabels)
    softmax = tf.nn.softmax(logits)
