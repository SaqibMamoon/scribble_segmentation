...
def most_recent_recursion(folder):
    ...
    for file in glob.glob(os.path.join(folder, 'recursion_*_data.hdf5')):
        ...
        logging.info(str(recursion))
        if recursion > max_recursion:
            
            max_recursion = np.copy(recursion)
            logging.info(str(max_recursion))
...
def create_recursion_dataset(target_folder, recursion):
    ...
    new_data_file.create_dataset('predicted_pre_cor', dtype='uint8', shape=base_data_file['scribbles_train'].shape)
    new_data_file['predicted_pre_cor'].attrs.create('processed', False, dtype='bool')
    new_data_file['predicted_pre_cor'].attrs.create('processed_to', 0, dtype='uint16')
    
    new_data_file.create_dataset('means', dtype='float', shape=(base_data_file['scribbles_train'].shape[0],base_data_file['scribbles_train'].shape[1],base_data_file['scribbles_train'].shape[2],5))
    new_data_file['means'].attrs.create('processed', False, dtype='bool')
    new_data_file['means'].attrs.create('processed_to', 0, dtype='uint16')

    new_data_file.create_dataset('stds', dtype='float', shape=(base_data_file['scribbles_train'].shape[0],base_data_file['scribbles_train'].shape[1],base_data_file['scribbles_train'].shape[2],5))
    new_data_file['stds'].attrs.create('processed', False, dtype='bool')
    new_data_file['stds'].attrs.create('processed_to', 0, dtype='uint16')
