...
def inference(images,keep_prob, inference_handle, nlabels, training=True):
    '''
    Wrapper function to provide an interface to a model from the model_zoo inside of the model module. 
    '''
    tf.logging.set_verbosity(tf.logging.WARN)
    network_output, s = inference_handle(images, training, nlabels,keep_prob)
    tf.logging.set_verbosity(tf.logging.WARN)

    return network_output,s
...
def predict_final(images, net_out, nlabels):
    '''
    ...
    net_output = model_zoo.seperate_crf_rnn_pro(net_out,images, nlabels)
    

    return net_output

def predict_net_out(images, keep_prob,inference_handle, nlabels):
    '''
    Returns the prediction for an image given a network from the model zoo
    :param images: An input image tensor
    :param inference_handle: A model function from the model zoo
    ...
    net_output = model_zoo.prostate_deep_dropout(images, tf.constant(False, dtype=tf.bool), nlabels,keep_prob)
    

    return net_output

def predict_logits(images,keep_prob, inference_handle, nlabels):
    '''
    Returns the prediction for an image given a network from the model zoo
    :param images: An input image tensor
    :param inference_handle: A model function from the model zoo
    ...
    logits = inference_handle(images, tf.constant(False, dtype=tf.bool), nlabels,keep_prob)
    softmax = tf.nn.softmax(logits)

    mask = tf.arg_max(softmax, dimension=-1)

    return mask, softmax,logits
