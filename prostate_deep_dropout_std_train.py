...
import acdc_data_dropout_pro as acdc_data
...
from experiments import prostate_deep_dropout_std_exp as exp_config
...
#                saver.restore(sess, init_checkpoint_path)
            ...
            saver.restore(sess,"/scratch_net/biwirender02/cany/scribble/logdir/prostate_deep_dropout_std_exp/recursion_0_model_best_dice.ckpt-4799")
...
def predict_next_gt(data,
    ...
    if not processed:
        ...
        for scr_idx in range(processed_to, scr_max, exp_config.batch_size):
            ...
            temp_std = np.squeeze(np.std(temp,axis=0))
   
            conf = 1
            
            max_vals = temp_mean + conf*temp_std
            min_vals = temp_mean - conf*temp_std
            ...
            for t in range(exp_config.batch_size):
                for m in range(212):
                    for n in range(212):
                        ...
                        min_val = min_vals[t,m,n,estim] 
                        temp = np.squeeze(np.copy(max_vals[t,m,n,:]))
                        temp[estim] = 0
                        best_ind = np.argmax(temp)
                        if temp[best_ind] > min_val:
                            mask_out[t,m,n] = 0
...
#                    min_vals = cur_vals - conf*cur_stds
            ...
            for indice in range(len(ind)):
                ...
                means[ind[indice], ...] = np.squeeze(temp_mean[indice, ...])
                stds[ind[indice], ...] = np.squeeze(temp_std[indice, ...])
