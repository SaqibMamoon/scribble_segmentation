...
from acdc_data_dropout import most_recent_recursion
...
def score_data(input_folder, output_folder, model_path, exp_config, do_postprocessing=False, recursion=None,apply_crf=False):
    ...
    mask_pl, softmax_pl = model.predict(images_pl, keep_prob,exp_config.model_handle, exp_config.nlabels)
...
#        saver.restore(sess,"/scratch_net/biwirender02/cany/scribble/logdir/heart_dropout_welch_non_exp2/recursion_2_model_best_xent.ckpt-34999")
#            
...
#                                logits_out = average_outputs(sess,softmax_pl,images_pl,network_input,keep_prob)
##                                
                                logits_out = sess.run(softmax_pl,feed_dict={images_pl: network_input,keep_prob:1})
                                if apply_crf:
                                    softmax = sess.run(tf.nn.softmax(logits_out))
                                    logits_out = do_crf(np.copy(softmax),network_input)
                                
                    #            mask_out, logits_out = sess.run([mask_pl, softmax_pl], feed_dict={images_pl: network_input,keep_prob:1})
...
if __name__ == '__main__':
    ...
    model_path = '/scratch_net/biwirender02/cany/scribble/logdir/heart_dropout_test'
