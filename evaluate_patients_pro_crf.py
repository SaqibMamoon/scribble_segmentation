    ...
    with tf.Session(config = config) as sess:
        ...
        for patient in range(len(slices_val)):
            ...
            for sli in range(int(slices_val[patient])):
                ...
                x=np.copy(np.expand_dims(np.expand_dims(images[int(sli+cur_slice),:,:],axis=3),axis=0))
    #            y=labels[int(sli+cur_slice),:,:]
                
    
                softmax = tf.nn.softmax(logits_op)
                
                feed_dict = { images_placeholder: x,
                  
                  training_time_placeholder: False}
        
                mask = sess.run(softmax, feed_dict=feed_dict)
    
                
                res[sli,:,:] = np.squeeze(apply_crf(np.copy(mask),x)) 
...
#            logging.info('saving to: %s' % image_file_name)
        ...
        logging.info('Dices : ' + str(np.mean(dices,axis=0))) 
