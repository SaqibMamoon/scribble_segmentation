...
from experiments import prostate_full_sup_crf as exp_config
...
#        with tf.Graph().as_default():
        with tf.Session(config = config) as sess:
            ...
            saver.restore(sess,'/scratch_net/biwirender02/cany/scribble/logdir/prostate_full_sup_new_crf/recursion_1_model_best_dice.ckpt-6599')
            ...
            [val_loss, val_dice, hard_pred, labels, cdice] = do_eval(sess,
                                           ...
                                           images_val,
                                           labels_val,
                                           exp_config.batch_size,slices_val,validation_res_path)
...
#            dice = dc(gt_c_i, pred_c_i)
        ...
        save_nii(out_file_name, np.uint8(np.asarray(res)))
        ...
        save_nii(image_file_name, np.uint8(np.asarray(255*images[np.uint(cur_slice):np.uint(cur_slice+slices_val[patient]),:,:])))
