...
from experiments import prostate_full_super_new_exp as exp_config
...
def save_specimen(images_train, labels_train, images_val, labels_val):
    sio.savemat( validation_res_path+ '/sample.mat', {'train_image':np.float32(images_train[50,:,:]),
                    'labels_train':np.float32(labels_train[50,:,:]), 'val_image':np.float32(images_val[50,:,:]),
                    'labels_val':np.float32(labels_val[50,:,:])})
...
#        with tf.Graph().as_default():
        ...
        save_specimen(images_train, labels_train, images_val, labels_val)
